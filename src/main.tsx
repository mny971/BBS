...
export default function AppMain() {
    ...
    const [view, setView] = useState<'customer' | 'trips' | 'dashboard' | 'profile' | 'about' | 'admin' | 'auth'>('auth');
    ...
    const NavButton = ({ target, icon: Icon, label }: { target: typeof view, icon: any, label: string }) => (
        <button
            ...
            className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-all duration-300 ${view === target ? 'text-primary' : 'text-gray-400 hover:text-gray-600'
                }`}
        >
    ...
    return (
        <>
            ...
            {view === 'auth' && <AuthScreen userRole={userRole} onAuthenticated={() => { }} />}
        </>
    )
}